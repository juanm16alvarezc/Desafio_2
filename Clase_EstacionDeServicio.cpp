...
void EstacionDeServicio:: consultarHistoricoTransacciones(int pos) const {
    if (pos < 0 || pos >= numSurtidores) {
        cout << "Surtidor no encontrado." << endl;
        return;
    }
    surtidores[pos]->mostrarHistoricoVentas();
}
...
void EstacionDeServicio::simularVenta( RedNacional& red) {
    ...
    unsigned int litrosVendidos = 3 + rand() % 18;
    string categoria;
    ...
    litrosVendidos=tanque.reducirGasolina(litrosVendidos,categoria);
